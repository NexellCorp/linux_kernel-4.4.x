...
static int _set_phys_from_fd(struct nx_deinterlacer *me,
	...
	int src_y_dma_fd, dst_y_dma_fd;
	int src_cb_dma_fd, dst_cb_dma_fd;
	int src_cr_dma_fd, dst_cr_dma_fd;

	dma_addr_t src_y_addr, dst_y_addr;
	dma_addr_t src_cb_addr, dst_cb_addr;
	dma_addr_t src_cr_addr, dst_cr_addr;
	...
	/* set pysical address from DMA fd for source buffers. */
	for (i = 0; i < 3; i++) {
		if (frame_info->src_bufs[i].plane_num == 1) {
			src_y_dma_fd = frame_info->src_bufs[i].plane3.fds[0];
			y_stride = frame_info->src_bufs[i].plane3.src_stride[0];
			c_stride = frame_info->src_bufs[i].plane3.src_stride[1];

			if (src_y_dma_fd > 0) {
				ret = get_phy_addr_from_fd(dev, src_y_dma_fd,
						true, &src_y_addr);
				frame_info->src_bufs[i].plane3.phys[0] =
					src_y_addr;
				frame_info->src_bufs[i].plane3.phys[1] =
					src_y_addr + (y_stride * height);
				frame_info->src_bufs[i].plane3.phys[2] =
					frame_info->src_bufs[i].plane3.phys[1] +
					(c_stride * (height >> 1));
			}
		} else if (frame_info->src_bufs[i].plane_num == 3) {
			src_y_dma_fd = frame_info->src_bufs[i].plane3.fds[0];
			src_cb_dma_fd = frame_info->src_bufs[i].plane3.fds[1];
			src_cr_dma_fd = frame_info->src_bufs[i].plane3.fds[2];
			...
			if (src_cb_dma_fd > 0) {
				ret = get_phy_addr_from_fd(dev, src_cb_dma_fd,
					true, &src_cb_addr);
				frame_info->src_bufs[i].plane3.phys[1] =
					src_cb_addr;
			}
			...
			if (src_cr_dma_fd > 0) {
				ret = get_phy_addr_from_fd(dev, src_cr_dma_fd,
					true, &src_cr_addr);
				frame_info->src_bufs[i].plane3.phys[2] =
					src_cr_addr;
			}
