...
static int alloc_dma_buffer(struct nx_decimator *me)
{
	if (me->buf.addr == NULL) {
		...
		if (me->buf.format == MEDIA_BUS_FMT_YVYU12_1X24) {
			...
			y_size = buf->stride[0] * ALIGN(me->dst_height, 16);
			cbcr_size = 0;
		} else {
			     ...
			     const struct v4l2_crop *crop)
{
	...
	if (!ret) {
		pr_debug("[%s] crop %d:%d:%d:%d\n", __func__, crop->c.left,
				crop->c.top, crop->c.width, crop->c.height);
		me->clip_width = crop->c.width;
		me->clip_height = crop->c.height;
		if ((me->src_width == me->dst_width) &&
				(me->src_height == me->dst_height)) {
			me->dst_width = me->clip_width;
			me->dst_height = me->clip_height;
		}
	}
	return 0;
}
