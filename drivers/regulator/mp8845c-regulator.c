...
static void asv_core_setup(struct mp8845c_regulator *ri,
	struct mp8845c_regulator_platform_data *mp8845c_pdata)
{
	...
	if (clk != NULL) {
		clk_rate = clk_get_rate(clk);
		{
			...
			idslv = (i != ASV_CORE_ARRAY_SIZE) ? i
				: (ASV_CORE_ARRAY_SIZE-1);
			...
			if (asvlv <= (ASV_CORE_ARRAY_SIZE-1))
				mp8845c_pdata->init_uV =
					asv_core_tables[asvlv].uV;

			dev_info(ri->dev, "IDS(%dmA) RO(%d) ASV(%d) Vol(%duV) CLK(%luHz)\n",
				ids, ro, asvlv,
				mp8845c_pdata->init_uV, clk_rate);
		}
	}
}
#endif
...
static int __mp8845c_set_voltage(struct mp8845c_regulator *ri,
	int min_uV, int max_uV, unsigned *selector)
{
	...
	vout_val = (ri->vout_en << ri->en_bit) |
		(ri->vout_reg_cache & ~ri->vout_mask) |
		(vsel & ri->vout_mask);

	mp8845c_set_bits(ri->client, MP8845C_REG_SYSCNTL2,
		(1 << MP8845C_POS_GO));
...
static int mp8845c_set_voltage_time_sel(struct regulator_dev *rdev,
...
#if 1
static int mp8845c_set_voltage_sel(struct regulator_dev *rdev,
	unsigned selector)
{
	struct mp8845c_regulator *ri = rdev_get_drvdata(rdev);
	int uV;
...
#define MP8845C_REG(_name, _id, _name_id, _en_reg, \
				_en_bit, _vout_reg, _vout_mask, \
				_min_mv, _max_mv, _step_uV, _nsteps, \
				_delay, _cmd_delay, _ops) \
{ \
	.name			= #_name, \
...
static int mp8845c_regulator_preinit(struct mp8845c_regulator *ri,
	struct mp8845c_regulator_platform_data *mp8845c_pdata)
{
	int ret = 0;
	...
	mp8845c_set_bits(ri->client, MP8845C_REG_SYSCNTL1,
		(1 << MP8845C_POS_MODE));

	if (mp8845c_pdata->init_enable) {
		ret = mp8845c_set_bits(ri->client, ri->reg_en_reg,
			(1 << ri->en_bit));
		...
		ret = mp8845c_clr_bits(ri->client, ri->reg_en_reg,
			(1 << ri->en_bit));
		if (ret < 0)
...
static int mp8845c_regulator_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_regulators; i++) {
		...
		mp8845c_regulators[i].rdev = regulator_register(&mp8845c->desc,
			&config);
