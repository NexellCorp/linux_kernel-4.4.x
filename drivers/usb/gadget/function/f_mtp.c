...
#ifdef CONFIG_COMPAT
#define MTP_SEND_EVENT_COMPAT	0x40084d03
static long mtp_compat_ioctl(struct file *fp, unsigned int code, unsigned long value)
{
	struct mtp_dev *dev = fp->private_data;
	struct file *filp = NULL;
	int ret = -EINVAL;
	...
	case MTP_SEND_FILE_WITH_HEADER:
	{
		...
		if (copy_from_user(&mfr, compat_ptr(value), sizeof(mfr))) {
			ret = -EFAULT;
			goto fail;
		}
		/* hold a reference to the file while we are working with it */
	...
	case MTP_SEND_EVENT_COMPAT:
	{
		 ...
		 * which would interfere with bulk transfer state.
		 */
		if (copy_from_user(&compat_event, compat_ptr(value),
				   sizeof(struct compat_event))) {
			ret = -EFAULT;
		} else {
			struct mtp_event	event;
			unsigned long ldata;

			event.length = compat_event.length;
			ldata = compat_event.data;
			event.data = (void *)ldata;
			ret = mtp_send_event(dev, &event);
		}
